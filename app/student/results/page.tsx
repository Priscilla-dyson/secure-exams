...
export default function StudentResults() {
  ...
  const missedCount = missedExams.length
  ...
  return (
    <div className="space-y-6">
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Stat icon={Hourglass} label="Pending" value={0} accent="warning" />
      ...
      {tab === "released" && (
        results.filter(r => r.published).length === 0 ? (
          <div className="text-center py-12 border border-border rounded-md bg-card">
            ...
            <p className="text-muted-foreground">No released results</p>
          </div>
        ) : (
          <ReleasedList results={results.filter(r => r.published)} />
        )
      )}
        ...
        missedExams.length === 0 ? (
          ...
          <div className="space-y-3">
            {missedExams.map((exam) => (
              <div key={exam.id} className="rounded-md border border-destructive/20 bg-destructive/5 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-foreground">{exam.module.name}</p>
                    <p className="text-xs text-muted-foreground">{exam.title}</p>
                    <p className="text-xs text-destructive mt-1">You missed this exam. Contact your lecturer if you have a valid reason.</p>
                  </div>
          ...
          <div key={r.id} className="rounded-md border border-border bg-card">
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-foreground">{r.exam.module.name}</p>
                <p className="text-xs text-muted-foreground">{r.exam.title} · {new Date(r.createdAt).toLocaleDateString()}</p>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="font-mono text-base font-bold text-foreground">{r.score}<span className="text-muted-foreground">/{r.totalMarks}</span></p>
                  <p className="text-[11px] text-muted-foreground">{pct}%</p>
                </div>
                {r.grade && <span className="rounded bg-primary/10 px-2 py-1 text-xs font-bold text-primary">{r.grade}</span>}
            ...
            {isOpen && (
              <div className="border-t border-border px-4 py-4">
                ...
                <div className="mb-4 rounded-md bg-muted/50 p-3">
                  ...
                  <p className="text-sm text-muted-foreground">Your performance has been recorded.</p>
                </div>
              </div>
            )}
          </div>
...
function Stat({ icon: Icon, label, value, accent }: { icon: any; label: string; value: number; accent?: "warning" | "destructive" }) {
  const tone = accent === "warning" ? "text-[color:var(--warning)] bg-[color:var(--warning)]/10"
